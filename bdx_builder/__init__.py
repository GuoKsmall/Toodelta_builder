from tooldelta import game_utils, plugin_entry, Plugin, ToolDelta, Player, Chat, FrameExit,fmts,TYPE_CHECKING
...
class BDX_BDump(Plugin):
    ...
    def dump_bdx_menu(self, _):
        ...
        try:
            ...
            fmts.print_err(f"读取 {bdx_file} 出现问题: {err}")
            return
        bdx_name = bdx_file[:-4]
        fmts.print_inf(f"{bdx_name} 的导入已经开始 (进度条显示于游戏内)")
